...
bool EiEvents::on(EiEvent event, EiEventHandler handler) {
  ...
  if (event == EiEvent::None || index >= static_cast<uint16_t>(EiEvent::Count) || handler == nullptr) {
    logError(LS, ET::SYSTEM, "Invalid event subscription request.");
    return false;
  }
  for (uint8_t i = 0; i < MAX_HANDLERS_PER_EVENT; i++) {
    if (_handlers[index][i] == handler) {
      logInfo(LS, ET::SYSTEM, "Event subscription already exists.");
      return true;
    }
    if (_handlers[index][i] == nullptr) {
      _handlers[index][i] = handler;
      logInfo(LS, ET::SYSTEM, "Event subscription added: " + String(eventToString(event)));
      return true;
    }
  }
  logError(LS, ET::SYSTEM, "Event subscription limit exceeded.");
  return false;
}
...
bool EiEvents::off(EiEvent event, EiEventHandler handler) {
  ...
  for (uint8_t i = 0; i < MAX_HANDLERS_PER_EVENT; i++) {
    if (_handlers[index][i] == handler) {
      ...
      logInfo(LS, ET::SYSTEM, "Event subscription removed.");
      return true;
    }
  }
  return false;
